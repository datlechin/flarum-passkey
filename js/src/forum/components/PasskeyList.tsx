...
export default class PasskeyList extends Component<IPasskeyListAttrs> {
    ...
    return (
      <div className="PasskeyList">
        {this.passkeys.length === 0 ? (
          <div className="PasskeyList--empty">
            {app.translator.trans('datlechin-passkey.forum.settings.list.empty')}
          </div>
        ) : (
        ...
        <div className="PasskeyList-actions">
          ...
          {this.passkeys.length > 0 && (
            <Button
              ...
              onclick={this.bulkRevoke.bind(this)}
            >
  ...
  protected itemInfo(passkey: Passkey): ItemList<Mithril.Children> {
    ...
    const sub = passkey.authenticatorName()
      ?? (passkey.backupState()
    ...
    items.add(
      'meta',
      <div className="PasskeyList-item-meta">{this.metaItems(passkey).toArray()}</div>,
      20
    );
  ...
  protected metaItems(passkey: Passkey): ItemList<Mithril.Children> {
    ...
    if (createdAt) {
      items.add(
        'added',
        <span>
          {app.translator.trans('datlechin-passkey.forum.settings.list.added_label')}{' '}
          <strong>{humanTime(createdAt)}</strong>
        </span>,
        20
      );
    }
    ...
    items.add(
      ...
      <span>
        {lastUsedAt ? (
          <>
            {app.translator.trans('datlechin-passkey.forum.settings.list.last_used_label')}{' '}
            <strong>{humanTime(lastUsedAt)}</strong>
          </>
        ) : (
  ...
  protected itemActions(passkey: Passkey): ItemList<Mithril.Children> {
    ...
    items.add(
      ...
      <Button
        ...
        icon="fas fa-pen"
        onclick={() => this.openRename(passkey)}
      >
    ...
    items.add(
      ...
      <Button
        ...
        icon="fas fa-trash"
        loading={!!this.loading[passkey.id()!]}
        onclick={() => this.revoke(passkey)}
      >
  ...
  protected async bulkRevoke(): Promise<void> {
    ...
    try {
      ...
      app.alerts.show(
        { type: 'success' },
        app.translator.trans('datlechin-passkey.forum.settings.alerts.bulk_revoke_succeeded')
      );
    } finally {
  ...
  protected async revoke(passkey: Passkey): Promise<void> {
    ...
    try {
      ...
      app.alerts.show(
        { type: 'success' },
        app.translator.trans('datlechin-passkey.forum.settings.alerts.revoke_succeeded')
      );
    } finally {
